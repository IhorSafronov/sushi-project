(() => {
    ...
    (() => {
        ...
        const testOrder = {
            ...
            utensils: "Тестовые приборы",
            info: "Дополнительная информация",
            delivery: "Тестовый способ доставки",
            payment: "Тестовый способ оплаты",
            productsMessage: "Тестовый продукт - $10",
            totalSum: "$10",
            distance: "10 км",
            deliveryPrice: "$5"
        };
        formData.push(testOrder);
        if (window.location.pathname.endsWith("/") || "/index.html" === window.location.pathname) document.addEventListener("DOMContentLoaded", (function() {
            var pageContent = document.body;
            var workData = JSON.parse(localStorage.getItem("workData")) || [];
            var lastValue = workData.length > 0 ? workData[workData.length - 1] : false;
            console.log(lastValue);
            if (lastValue) pageContent.classList.add("blurred");
            window.addEventListener("storage", (function(event) {
                if ("workData" === event.key) {
                    workData = JSON.parse(event.newValue) || [];
                    lastValue = workData.length > 0 ? workData[workData.length - 1] : false;
                    if (lastValue) pageContent.classList.add("blurred"); else pageContent.classList.remove("blurred");
                    workData.forEach((function(value) {
                        console.log(value);
                    }));
                }
            }));
        ...
        fetch("files/single-product.json").then((response => response.json())).then((data => {
            ...
            function createProductCard(product) {
                ...
                weight.textContent = `Вага: ${product.weight} г`;
        ...
        const headerBot = document.querySelector(".header__bot .body-bot");
        if (-1 !== window.location.href.indexOf("product-page.html?id=")) {
            const headerBotBody = document.querySelector(".header-bot__body");
            headerBotBody.classList.add("header-bot__column");
            const burgedBtn = document.querySelector(".icon-menu");
            burgedBtn.classList.add("burger-btn");
            const linkContainer = document.createElement("div");
            linkContainer.className = "link-container";
            const link1 = createAnchorLink("index.html#rolls", "Роли", "img/icon/rolls.svg");
            const link2 = createAnchorLink("index.html#maki", "Макі", "img/icon/maki.svg");
            const link3 = createAnchorLink("index.html#sets", "Сети", "img/icon/sets_2.svg");
            const link4 = createAnchorLink("index.html#sushi", "Суші", "img/icon/sushi.svg");
            const link5 = createAnchorLink("index.html#drinks", "Напої", "img/icon/drinks.svg");
            ...
            linkContainer.appendChild(link5);
            headerBot.insertBefore(linkContainer, headerBot.children[1]);
        }
        if (-1 !== window.location.href.indexOf("cart-page.html")) {
            const headerBotBody = document.querySelector(".header-bot__body");
            headerBotBody.classList.add("header-bot__column");
            const burgedBtn = document.querySelector(".icon-menu");
            burgedBtn.classList.add("burger-btn");
        ...
        function createAnchorLink(href, text, imgSrc) {
            const anchor = document.createElement("a");
            anchor.href = href;
            anchor.className = "anchors-body__item anchor-item";
            const img = document.createElement("img");
            img.src = imgSrc;
            img.alt = "icon";
            const imageContainer = document.createElement("div");
            imageContainer.className = "anchor-item__image";
            imageContainer.appendChild(img);
            const textContainer = document.createTextNode(text);
        ...
        links.forEach((link => {
            link.addEventListener("click", (e => {
                ...
                const offset = target.getBoundingClientRect().top + window.pageYOffset - 150;
                window.scrollTo({
                    top: offset,
                    behavior: "smooth"
                });
        ...
        if (window.location.pathname.endsWith("/") || "/index.html" === window.location.pathname) {
            window.addEventListener("scroll", (function() {
                ...
                if (scrollHeight > 300) backToTopButton.style.display = "block"; else backToTopButton.style.display = "none";
            }));
            const backToTopButton = document.querySelector(".back-to-top");
        ...
        if (window.location.pathname.endsWith("/") || "/index.html" === window.location.pathname) {
            window.addEventListener("scroll", (function() {
                const scrollHeight = window.pageYOffset;
                const callBackButton = document.querySelector(".call-back");
                if (scrollHeight > 300) callBackButton.style.display = "block"; else callBackButton.style.display = "none";
            }));
            const callBackButton = document.querySelector(".call-back");
            callBackButton.addEventListener("click", (function() {
                const phoneOne = document.querySelector(".call-back__phone-one");
                const phoneTwo = document.querySelector(".call-back__phone-two");
                phoneOne.classList.toggle("show");
                phoneTwo.classList.toggle("show");
            }));
        }
        if (window.location.href.includes("product-page.html?id=")) {
            window.onload = function() {
                const quantityInput = document.querySelector("[data-quantity-value]");
                let quantityValue = parseInt(quantityInput.value) || 1;
                if (null !== quantityInput) quantityInput.value = quantityValue;
            };
            const quantityInput = document.querySelector("[data-quantity-value]");
            const plusButton = document.querySelector("[data-quantity-plus]");
            const minusButton = document.querySelector("[data-quantity-minus]");
            const quantityContainer = document.querySelector("[data-quantity]");
            quantityInput.classList.add("quantity__input");
            plusButton.classList.add("quantity__button", "quantity__button_plus");
            minusButton.classList.add("quantity__button", "quantity__button_minus");
            quantityContainer.classList.add("quantity");
            let quantityValue = parseInt(quantityInput.value) || 1;
            quantityInput.value = quantityValue;
            quantityInput.addEventListener("input", (function() {
                this.value = this.value.replace(/\D/g, "");
                quantityValue = parseInt(this.value) || 1;
            }));
            quantityInput.addEventListener("keydown", (function(event) {
                if ("Enter" === event.key || "Backspace" === event.key) return;
            ...
            plusButton.addEventListener("click", (function() {
                quantityValue++;
                quantityInput.value = quantityValue;
                const idBtn = document.querySelector(".id-quantity").textContent;
                const priceBtn = document.querySelector(".price-quantity").textContent;
                productIds.push(idBtn);
                total += parseFloat(priceBtn);
                localStorage.setItem("productIds", JSON.stringify(productIds));
                localStorage.setItem("total", total);
                console.log(`ID товара: ${idBtn}, цена: ${priceBtn}, общая сумма: ${total}`);
            }));
            minusButton.addEventListener("click", (function() {
                if (quantityValue > 1) {
                    ...
                    const idBtn = document.querySelector(".id-quantity").textContent;
                    const priceBtn = document.querySelector(".price-quantity").textContent;
                    const index = productIds.indexOf(idBtn);
                    if (-1 !== index) productIds.splice(index, 1);
                    total -= parseFloat(priceBtn);
                    localStorage.setItem("productIds", JSON.stringify(productIds));
                    localStorage.setItem("total", total);
                    console.log(`ID товара: ${idBtn}, цена: ${priceBtn}, общая сумма: ${total}`);
        ...
        if (window.location.href.includes("product-page.html?id=")) {
            fetch("files/single-product.json").then((response => response.json())).then((data => {
                const productCardTemplate = document.querySelector("#product-card-template");
                const similarProductContainer = document.querySelector(".similar-product");
                const randomProductIds = getRandomProducts(data.products, 4);
                randomProductIds.forEach((productId => {
                    const product = data.products.find((p => p.id === productId));
                    const productElement = productCardTemplate.content.cloneNode(true);
                    productElement.querySelector(".product-id").textContent = product.id;
                    productElement.querySelector(".product-id-two").textContent = product.id;
                    productElement.querySelector(".product-name").textContent = product.title;
                    productElement.querySelector(".product-image").src = product.image;
                    productElement.querySelector(".product-weight").textContent = `${product.weight} г`;
                    productElement.querySelector(".product-price").textContent = `${product.price} грн.`;
                    productElement.querySelector(".price-console").textContent = product.price;
                    const addToCartButton = productElement.querySelector(".to-cart");
                    ...
                    similarProductContainer.appendChild(productElement);
                }));
            }));
            function getRandomProducts(products, count) {
                const productIds = products.map((p => p.id));
                const randomProductIds = [];
                while (randomProductIds.length < count) {
                    const randomIndex = Math.floor(Math.random() * productIds.length);
                    const randomProductId = productIds[randomIndex];
                    if (!randomProductIds.includes(randomProductId)) randomProductIds.push(randomProductId);
                }
        ...
        fetch("files/single-product.json").then((response => response.json())).then((data => {
            ...
            if (product) {
                ...
                const idQuantity = document.querySelector(".id-quantity");
                image.src = product.image;
                name.textContent = product.title;
                title.textContent = product.title;
                price.textContent = `${product.price}.00 ₴`;
                quantity.textContent = `Кількість: ${product.quantity}`;
                weight.textContent = `Вага: ${product.weight} г`;
                ingredients.textContent = "";
                id.textContent = product.id;
                idTwo.textContent = product.id;
                priceConsole.textContent = product.price;
                priceQuantity.textContent = product.price;
                idQuantity.textContent = product.id;
        ...
        const productLists = document.querySelectorAll(".product-list, .similar-product");
        ...
        updateCartTotal();
        window.addEventListener("scroll", (function() {
            var anchorsItems = document.querySelector(".anchors-items");
            if (!anchorsItems) return;
            var scrollTop = window.pageYOffset || document.documentElement.scrollTop;
            if (scrollTop > 200 && scrollTop > anchorsItems.offsetTop - anchorsItems.offsetHeight) anchorsItems.classList.add("anchors-items--sticky", "anchors-items--smooth"); else {
                anchorsItems.classList.remove("anchors-items--sticky");
                setTimeout((function() {
                    anchorsItems.classList.remove("anchors-items--smooth");
                }), 500);
            }
        }));
        window.addEventListener("load", (function() {
            var headerBot = document.querySelector(".header__bot");
            headerBot.classList.remove("header__bot--sticky");
        }));
        window.addEventListener("scroll", (function() {
            ...
            var scrollTop = window.pageYOffset || document.documentElement.scrollTop;
            if (scrollTop > headerHeight) headerBot.classList.add("header__bot--sticky"); else headerBot.classList.remove("header__bot--sticky");
        }));
        const mapApi = "https://maps.googleapis.com/maps/api/js?key=AIzaSyCJUnj6lhu0peJ9_Nds-YERCMVKl2_ytfg&libraries=places";
        ...
        const DESTINATION_LAT = 48.466539;
        const DESTINATION_LNG = 34.981295;
        let map;
        ...
        let cartTotal;
        let deliveryCost;
        function calculateDeliveryCost(cartTotal, distanceValue) {
            if (cartTotal < 400) return 0; else if (cartTotal >= 400 && cartTotal <= 800) if (distanceValue <= 5) return 0; else return 10 * (distanceValue - 5); else if (distanceValue <= 10) return 0; else return 10 * (distanceValue - 10);
        }
        function initAutocomplete() {
            ...
            autocomplete.addListener("place_changed", (() => {
                ...
                cartTotal = parseFloat(cartTotalElement.textContent.trim());
                observer = new MutationObserver((function(mutationsList) {
                    for (var mutation of mutationsList) if ("childList" === mutation.type || "characterData" === mutation.type) {
                        console.log("Цена в корзине изменилась");
                        updateDeliveryPrice();
                    }
                }));
                observerConfig = {
                ...
                observer.observe(cartTotalElement, observerConfig);
                cartTotalElement.addEventListener("DOMSubtreeModified", (() => {
                    console.log("Сумма в корзине изменилась");
                    cartTotal = parseFloat(cartTotalElement.textContent.trim());
                    updateDeliveryPrice();
                }));
                updateDeliveryPrice();
                if (cartTotal > 400) {
                    initializeMap(userLatLng, destinationLatLng);
                    ...
                    service.getDistanceMatrix({
                        ...
                        console.log("Расстояние: " + distanceText);
                        distanceElement.textContent = `Відстань для розрахунку доставки ${distanceText}`;
                        var distanceValue = parseFloat(distanceText.match(/\d+/)[0]);
                        var deliveryCost = calculateDeliveryCost(cartTotal, distanceValue);
                        deliveryPrice.textContent = `Ціна доставки: ${deliveryCost}`;
                        totalDeliveryPrice.textContent = `${deliveryCost}`;
                        console.log("Стоимость доставки:", deliveryCost, "грн");
                        updateDeliveryPrice();
                    }));
                } else {
                    console.log("Сумма в корзине меньше 400грн");
                    input.value = "";
                    deliveryPrice.textContent = "";
                }
            }));
            function updateDeliveryPrice() {
                var distanceText = distanceElement.textContent;
                var distanceValue = parseFloat(distanceText.match(/\d+/)?.[0] || 0);
                deliveryCost = calculateDeliveryCost(cartTotal, distanceValue);
                deliveryPrice.textContent = `Ціна доставки: ${deliveryCost} грн`;
                totalDeliveryPrice.textContent = `${deliveryCost} грн`;
            }
        }
        function initializeMap(userLatLng, destinationLatLng) {
        ...
        function calculateAndDisplayRoute(origin, destination) {
            ...
            directionsService.route({
                ...
                if (status === google.maps.DirectionsStatus.OK) directionsDisplay.setDirections(response); else console.log("Ошибка при построении маршрута: " + status);
            }));
        }
        if (-1 === window.location.pathname.indexOf("product-page.html")) {
        ...
        if (window.location.href.includes("cart-page.html")) {
            const productCardTemplate = document.querySelector("#card-template");
            function createProductCard(product, count) {
                const card = productCardTemplate.content.cloneNode(true);
                const image = card.querySelector(".product-image");
                const title = card.querySelector(".product-name");
                const price = card.querySelector(".product-price");
                ...
                const priceQuantity = card.querySelector(".price-quantity");
                const idQuantity = card.querySelector(".id-quantity");
                title.textContent = `${product.title} x${count}`;
                image.src = product.image;
                price.textContent = `${product.price * count}.00 `;
                weight.textContent = `Вага: ${product.weight} г.`;
                id.textContent = product.id;
            ...
            function updateProductCards() {
                fetch("files/single-product.json").then((response => response.json())).then((data => {
                    ...
                    if (savedProductIds) {
                        ...
                        const cartProducts = productIds.map((productId => data.products.find((product => product.id === productId))));
                        const productCounts = {};
                        cartProducts.forEach((product => {
                            if (productCounts[product.id]) productCounts[product.id]++; else productCounts[product.id] = 1;
                        }));
                        ...
                        if (productList) {
                            ...
                            cartProducts.forEach((product => {
                                ...
                                if (!addedProductIds.includes(product.id)) {
                                    const card = createProductCard(product, count);
                                    productList.appendChild(card);
            ...
            updateProductCards();
            function watchProductIdsChanges() {
                let savedProductIds = localStorage.getItem("productIds");
                setInterval((() => {
                    const currentProductIds = localStorage.getItem("productIds");
                    if (currentProductIds !== savedProductIds) {
                        savedProductIds = currentProductIds;
                        updateProductCards();
                    }
                }), 1e3);
            }
            watchProductIdsChanges();
            const cartList = document.querySelector(".cart-list");
            cartList.addEventListener("click", (event => {
                if (event.target.classList.contains("remove-btn")) {
                    const cartItem = event.target.closest(".cart-list__body");
                    const productId = cartItem.querySelector(".id-delete-btn").textContent;
                    const countdownEl = document.createElement("div");
                    countdownEl.classList.add("countdown");
                    const countdownText = document.createElement("div");
                    countdownText.classList.add("countdown-text");
                    countdownText.textContent = `Видалити товар?`;
                    const cancelButton = document.createElement("button");
                    cancelButton.classList.add("cancel-button");
                    cancelButton.textContent = "Відмінити";
                    countdownEl.appendChild(countdownText);
                    countdownEl.appendChild(cancelButton);
                    cartItem.appendChild(countdownEl);
                    const cancelButtonHandler = () => {
                        clearInterval(countdownInterval);
                        cartItem.removeChild(countdownEl);
                    };
                    cancelButton.addEventListener("click", cancelButtonHandler);
                    let count = 3;
                    const countdownInterval = setInterval((() => {
                        if (count > 0) {
                            countdownText.textContent = `Видалення через ${count} секунд`;
                            count--;
                        } else {
                            clearInterval(countdownInterval);
                            cartItem.removeChild(countdownEl);
                            const indices = [];
                            for (let i = 0; i < productIds.length; i++) if (productIds[i] === productId) indices.push(i);
                            if (indices.length > 0) {
                                for (let i = indices.length - 1; i >= 0; i--) productIds.splice(indices[i], 1);
                                localStorage.setItem("productIds", JSON.stringify(productIds));
                                cartItem.remove();
                                updateCartTotal();
                                console.log(`Все вхождения товара ${productId} удалены из корзины.`);
                            }
                        }
                    }), 1e3);
                }
        ...
        if (productNamesContainer) {
            ...
            fetch("files/single-product.json").then((response => response.json())).then((data => {
                function updateProductList() {
                    ...
                    productIds.forEach((productId => {
                        const product = data.products.find((p => p.id === productId));
                        if (product) if (products[product.id]) products[product.id].count++; else products[product.id] = {
                            title: product.title,
                            price: product.price,
                            count: 1
                        };
                    ...
                    for (const productId in products) {
                        const {title, price, count} = products[productId];
                        const listItem = document.createElement("li");
                        listItem.classList.add("product-list__item");
                        const titleElement = document.createElement("span");
                        titleElement.classList.add("product-list__title");
                        titleElement.textContent = `${title} x${count}`;
                        const sum = price * count;
                        totalPrice += sum;
                        const sumElement = document.createElement("span");
                        sumElement.classList.add("product-list__sum");
                        sumElement.textContent = `${sum} грн`;
                        listItem.appendChild(titleElement);
                        listItem.appendChild(sumElement);
                        productList.appendChild(listItem);
                    }
                    productNamesContainer.appendChild(productList);
                }
                updateProductList();
                function watchProductIds() {
                    setInterval((() => {
                        const newProductIds = JSON.parse(localStorage.getItem("productIds")) || [];
                        if (JSON.stringify(newProductIds) !== JSON.stringify(productIds)) {
                            console.log("Updated productIds:", newProductIds);
        ...
        const totalPriceBlocks = document.querySelectorAll(".total-price__summary");
        function updateTotalPrice() {
            const total = parseFloat(localStorage.getItem("total")) || 0;
            totalPriceBlocks.forEach((block => {
                block.textContent = `${total} грн`;
            }));
        }
        function watchTotal() {
            setInterval((() => {
                const newTotal = parseFloat(localStorage.getItem("total")) || 0;
                if (newTotal !== total) {
        ...
        if (window.location.href.includes("cart-page.html")) {
            ...
            cartList.addEventListener("click", (event => {
                ...
                if (plusButton) {
                    const productIdElement = plusButton.closest(".body-cart").querySelector(".id-quantity");
                    const productId = productIdElement.textContent;
                    const priceElement = plusButton.closest(".body-cart").querySelector(".price-quantity");
                    const price = parseFloat(priceElement.textContent);
                    let productIds = JSON.parse(localStorage.getItem("productIds")) || [];
                    let total = parseFloat(localStorage.getItem("total")) || 0;
                    productIds.push(productId);
                    total += price;
                    console.log(`Нажата кнопка "+", идентификатор товара: ${productId}, цена: ${price}`);
                    localStorage.setItem("productIds", JSON.stringify(productIds));
                ...
                if (minusButton) {
                    const productIdElement = minusButton.closest(".body-cart").querySelector(".id-quantity");
                    const productId = productIdElement.textContent;
                    const priceElement = minusButton.closest(".body-cart").querySelector(".price-quantity");
                    const price = parseFloat(priceElement.textContent);
                    let productIds = JSON.parse(localStorage.getItem("productIds")) || [];
                    let total = parseFloat(localStorage.getItem("total")) || 0;
        ...
        if (window.location.href.includes("cart-page.html")) {
            ...
            form.addEventListener("submit", (function(e) {
                ...
                const cutlerySelect = form.querySelector('select[name="cutlery"]');
                ...
                const utensils = cutlerySelect ? cutlerySelect.value : "";
                const distanceElement = document.getElementById("distance");
                const distance = distanceElement ? distanceElement.textContent : "";
                const deliveryPriceElement = document.getElementById("delivery-price");
                const deliveryPrice = deliveryPriceElement ? deliveryPriceElement.textContent : "";
                const productsList = document.querySelectorAll(".product-list__item");
                const products = [];
                for (let product of productsList) {
                    const title = product.querySelector(".product-list__title").textContent;
                    const price = product.querySelector(".product-list__sum").textContent;
                    products.push({
                        title,
                        price
                    });
                }
                const productsMessage = products.map(((product, index) => `${index + 1}. ${product.title} - ${product.price}`)).join("\n");
                const totalSum = document.querySelector(".total-price__summary").textContent;
                let orderNumber;
                if (formData.length > 0) orderNumber = formData.length + 1; else orderNumber = 1;
                const orderData = {
                    orderNumber,
                    dateTime: (new Date).toLocaleString("uk-UA"),
                    name,
        ...
        if (window.location.href.includes("welcome-page.html")) document.addEventListener("DOMContentLoaded", (function() {
            var orderNumberElement = document.getElementById("order-number");
            var dateTimeElement = document.getElementById("date-time");
            var nameElement = document.getElementById("name");
            var phoneElement = document.getElementById("phone");
            var addressElement = document.getElementById("address");
            var utensilsElement = document.getElementById("utensils");
            var infoElement = document.getElementById("info");
            var deliveryElement = document.getElementById("delivery");
            var paymentElement = document.getElementById("payment");
            var productsMessageElement = document.getElementById("products-message");
            var totalSumElement = document.getElementById("total-sum");
            var distanceElement = document.getElementById("distance-welcome");
            var deliveryPriceElement = document.getElementById("delivery-price-welcome");
            var formData = JSON.parse(localStorage.getItem("formData"));
            if (formData && formData.length > 0) {
                var lastOrderData = formData.reverse()[0];
                orderNumberElement.textContent = lastOrderData.orderNumber;
                dateTimeElement.textContent = lastOrderData.dateTime;
                nameElement.textContent = lastOrderData.name;
                phoneElement.textContent = lastOrderData.phone;
                addressElement.textContent = lastOrderData.address;
                utensilsElement.textContent = lastOrderData.utensils;
                infoElement.textContent = lastOrderData.info;
                deliveryElement.textContent = lastOrderData.delivery + "грн.";
                paymentElement.textContent = lastOrderData.payment + "грн.";
                var productsArray = lastOrderData.productsMessage.split("\n");
                ...
                productsMessageElement.appendChild(productsList);
                totalSumElement.textContent = lastOrderData.totalSum;
                deliveryPriceElement.textContent = lastOrderData.deliveryPrice;
            } else {
                orderNumberElement.textContent = "Наме даних про замовлення";
                dateTimeElement.style.display = "none";
                nameElement.style.display = "none";
                phoneElement.style.display = "none";
                addressElement.style.display = "none";
                utensilsElement.style.display = "none";
                infoElement.style.display = "none";
                deliveryElement.style.display = "none";
                paymentElement.style.display = "none";
                productsMessageElement.style.display = "none";
        ...
        function watchLocalStorageChanges() {
            window.addEventListener("storage", (event => {
                if ("productIds" === event.key) {
                    const newProductIds = JSON.parse(event.newValue) || [];
                    console.log("Updated productIds:", newProductIds);
                } else if ("total" === event.key) {
                    const newTotal = parseFloat(event.newValue) || 0;
                    console.log("Updated total:", newTotal);
                }
            }));
        }
        watchLocalStorageChanges();
        if (window.location.href.includes("cart-page.html")) document.addEventListener("DOMContentLoaded", (function() {
            const deliveryPriceElement = document.querySelector(".total-delivery-price");
            const mainTotalElement = document.querySelector(".main-total");
            let totalCart = parseFloat(localStorage.getItem("total")) || 0;
            let deliveryPriceTotal = parseFloat(deliveryPriceElement.textContent.trim()) || 0;
            const updateTotalAmount = function() {
                const totalAmount = totalCart + deliveryPriceTotal;
                mainTotalElement.textContent = `${totalAmount} грн`;
            };
            const observerCallback = function(mutationsList) {
                for (let mutation of mutationsList) if ("childList" === mutation.type || "characterData" === mutation.type) {
                    const content = deliveryPriceElement.textContent.trim();
                    if ("" !== content) {
                        deliveryPriceTotal = parseFloat(content);
                        updateTotalAmount();
                    }
                }
            };
            ...
            observer.observe(deliveryPriceElement, observerConfig);
            updateTotalAmount();
            setInterval((function() {
                totalCart = parseFloat(localStorage.getItem("total")) || 0;
                updateTotalAmount();
            }), 1e3);
        }));
        if (window.location.href.includes("admin.html")) {
            document.addEventListener("DOMContentLoaded", (function() {
                ...
                var pageContent = document.body;
                var workData = JSON.parse(localStorage.getItem("workData")) || [];
                var isBlurred = toggleButton.classList.contains("blurred");
                console.log("isBlurred:", isBlurred);
                toggleButton.addEventListener("click", (function() {
                    pageContent.classList.toggle("blurred");
                    isBlurred = pageContent.classList.contains("blurred");
                    toggleButton.textContent = isBlurred ? "Выключить блюр" : "Включить блюр";
                    console.log("isBlurred:", isBlurred);
                    workData.push(isBlurred);
                    localStorage.setItem("workData", JSON.stringify(workData));
                    console.log("workData:", workData);
                }));
            }));
            window.addEventListener("DOMContentLoaded", (function() {
                var passwordFormContainer = document.getElementById("password-form-container");
                var adminContent = document.getElementById("admin-content");
                var passwordInput = document.getElementById("password-input");
                var submitButton = document.getElementById("submit-password");
                var errorMessage = document.getElementById("error-message");
                var logoutButton = document.getElementById("logout-button");
                var correctPassword = "safr1404";
                ...
                submitButton.addEventListener("click", (function() {
                    ...
                    if (enteredPassword === correctPassword) showAdminContent(); else showErrorMessage("Неверный пароль");
                }));
                logoutButton.addEventListener("click", (function() {
                    endSession();
                }));
                var storedSessionActive = localStorage.getItem("sessionActive");
                if (storedSessionActive && "true" === storedSessionActive) showAdminContent(); else endSession();
            ...
            document.getElementById("search-button").addEventListener("click", (function() {
                ...
                var dateFromInput = document.getElementById("date-from-input").value;
                var dateToInput = document.getElementById("date-to-input").value;
                console.log("Date From:", dateFromInput);
                console.log("Date To:", dateToInput);
                if ("" === dateFromInput || "" === dateToInput) {
                    console.log("Привет. Нет даты");
                    var defaultDateFrom = new Date("2023-01-01").toLocaleString("uk-UA");
                    var defaultDateTo = (new Date).toLocaleString("uk-UA");
                    dateFromInput = dateFromInput || defaultDateFrom;
                    dateToInput = dateToInput || defaultDateTo;
                }
                dateFromInput = new Date(dateFromInput).toLocaleString("uk-UA");
                dateToInput = new Date(dateToInput).toLocaleString("uk-UA");
                for (var i = 0; i < formData.length; i++) {
                    var order = formData[i];
                    var orderDate = order.dateTime;
                    if (dateFromInput <= orderDate && orderDate <= dateToInput) if ("замовники" === searchInput) {
                        createCategoryItem("Name: ", order.name);
                        createCategoryItem("Phone: ", order.phone);
                        createCategoryItem("Date: ", order.dateTime);
                    } else if ("суми" === searchInput) createCategoryItem("Total Sum: ", order.totalSum); else if ("номер" === searchInput) createCategoryItem("Order number ", order.orderNumber); else if ("все" === searchInput) {
                        createCategoryItem("Order number: ", order.orderNumber);
                        createCategoryItem("Name: ", order.name);
                        createCategoryItem("Phone: ", order.phone);
                        createCategoryItem("Date: ", order.dateTime);
                        createCategoryItem("Distance: ", order.distance);
                        createCategoryItem("Disctance price: ", order.deliveryPrice);
                    } else if ("адреса" === searchInput) createCategoryItem("Address: ", order.address); else if ("суши" === searchInput) {
                        if (order.productsMessage) {
                            createCategoryItem("Products:", "");
                            var products = order.productsMessage.split("\n");
                            for (var j = 0; j < products.length; j++) createCategoryItem("", products[j]);
                        }
                    } else if ("инфо" === searchInput) {
                        createCategoryItem("Payment: ", order.payment);
                        createCategoryItem("Delivery: ", order.delivery);
                        createCategoryItem("Dishes: ", order.utensils);
                    }
                }
                function createCategoryItem(title, text) {
                    var itemElement = document.createElement("p");
                    itemElement.classList.add("category-item");
                    var titleElement = document.createElement("p");
                    titleElement.classList.add("category-title-admin");
                    titleElement.textContent = title;
                    var textElement = document.createElement("p");
                    textElement.classList.add("category-text-admin");
