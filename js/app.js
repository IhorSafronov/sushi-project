(() => {
    ...
    (() => {
        ...
        if (window.location.href.includes("cart-page.html")) {
            ...
            console.log("Product IDs:", savedProductIds);
            if (savedProductIds) {
            ...
            cartList.addEventListener("click", (event => {
                if (event.target.classList.contains("remove-btn")) {
                    ...
                    if (index > -1) {
                        ...
                        countdownText.textContent = `Ви хочете видалити цей товар?`;
                        const cancelButton = document.createElement("button");
                        cancelButton.classList.add("cancel-button");
                        cancelButton.textContent = "Повернути?";
                        ...
                        const cancelButtonHandler = () => {
                            ...
                            console.log(`Удаление товара ${productId} отменено.`);
                            productIds.splice(index, 0, productId);
                            localStorage.setItem("productIds", JSON.stringify(productIds));
                        };
                        cancelButton.addEventListener("click", cancelButtonHandler);
                        let count = 3;
                        const countdownInterval = setInterval((() => {
                            if (count > 0) countdownText.textContent = `Відалення через ${count--} секунд`; else {
                                clearInterval(countdownInterval);
                                cartItem.removeChild(countdownEl);
                                cartItem.remove();
                                updateCartTotal();
            ...
            function updateCartTotal() {
                ...
                if (savedProductIds) {
                    ...
                    fetch("files/single-product.json").then((response => response.json())).then((data => {
                        ...
                        localStorage.setItem("total", total);
                        let savedTotal = parseFloat(localStorage.getItem("total")) || 0;
            ...
            function updatePageData() {
                ...
                if (savedProductIds) {
                    ...
                    fetch("files/single-product.json").then((response => response.json())).then((data => {
                        ...
                        const titleProducts = cartProducts.map((product => product.title));
                        const priceProducts = cartProducts.map((product => product.price));
                        const productNamesContainer = document.querySelector(".products-names");
                        if (productNamesContainer) {
                            productNamesContainer.innerHTML = "";
                            const productNamesList = document.createElement("ul");
                            productNamesList.classList.add("list-product");
            ...
            if (window.location.href.includes("cart-page.html")) {
                ...
                if (null !== cardTemplate) window.onload = function() {
                    const productContainer = document.getElementById("product-container");
                    productContainer.addEventListener("click", (function(event) {
                        if (event.target.matches("[data-quantity-plus]")) {
                            const plusButton = event.target;
                            const productCard = plusButton.closest(".body-cart");
                            if (null !== productCard) {
                                const quantityInput = productCard.querySelector("[data-quantity-value]");
                                const idBtn = productCard.querySelector(".id-quantity").textContent;
                                const priceBtn = productCard.querySelector(".price-quantity").textContent;
                                let quantityValue = parseInt(quantityInput.value) || 1;
                                quantityValue++;
                                quantityInput.value = quantityValue;
                                productIds.push(idBtn);
                    ...
                    productContainer.addEventListener("click", (function(event) {
                        if (event.target.matches("[data-quantity-minus]")) {
                            const minusButton = event.target;
                            const productCard = minusButton.closest(".body-cart");
                            if (null !== productCard) {
                                const quantityInput = productCard.querySelector("[data-quantity-value]");
                                const idBtn = productCard.querySelector(".id-quantity").textContent;
                                const priceBtn = productCard.querySelector(".price-quantity").textContent;
                                ...
                                if (quantityValue > 1) {
                                    ...
                                    console.log(`ID товара: ${idBtn}, цена: ${priceBtn}, общая сумма: ${total}`);
                                    const productNamesContainer = document.querySelector(".products-names");
                                    if (productNamesContainer) productNamesContainer.style.display = "none";
                                    updatePageData();
                                }
                            }
                        }
            ...
            if (window.location.href.includes("cart-page.html")) document.addEventListener("DOMContentLoaded", (function() {
                ...
                form.addEventListener("submit", (function(e) {
                    ...
                    try {
                        ...
                        if (!nameInput || !phoneInput) throw new Error("Не удалось получить элементы формы");
                        const name = nameInput.value.trim();
                        const phone = phoneInput.value.trim();
                        addressInput.value.trim();
                        if (!name || !phone) throw new Error("Не заполнены обязательные поля формы");
                        const formData = new FormData(form);
                        let formDataArray = JSON.parse(localStorage.getItem("formData")) || [];
                        formDataArray.push(JSON.stringify(Object.fromEntries(formData.entries())));
                ...
                function updateFormDataCount() {
                    const formData = JSON.parse(localStorage.getItem("formData")) || [];
                    console.log(`Количество объектов в localStorage: ${formData.length}`);
                    console.log("Содержимое localStorage:", formData);
                }
                updateFormDataCount();
                window.addEventListener("storage", (function(event) {
                    if ("formData" === event.key) updateFormDataCount();
                }));
            }));
