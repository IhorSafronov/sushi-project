        ...
        if (window.location.href.includes("cart-page.html")) {
            ...
            if (savedProductIds) {
                ...
                fetch("files/single-product.json").then((response => response.json())).then((data => {
                    ...
                    function createProductCard(product, count) {
                        ...
                        title.textContent = `${product.title} (${count})`;
                        image.src = product.image;
                        price.textContent = `${product.price * count}.00 `;
                        weight.textContent = `Вага: ${product.weight} г.`;
                        quantity.textContent = `Кількість: ${count} шт.`;
                        id.textContent = product.id;
            ...
            cartList.addEventListener("click", (event => {
                if (event.target.classList.contains("remove-btn")) {
                    ...
                    let productIds = JSON.parse(localStorage.getItem("productIds")) || [];
                    const indexes = [];
                    for (let i = 0; i < productIds.length; i++) if (productIds[i] === productId) indexes.push(i);
                    if (indexes.length > 0) {
                        for (let i = indexes.length - 1; i >= 0; i--) productIds.splice(indexes[i], 1);
                        localStorage.setItem("productIds", JSON.stringify(productIds));
                        console.log(`Product ID ${productId} removed from productIds.`);
                        const cartItems = event.target.closest(".cart-list__body").parentNode;
                        cartItems.querySelectorAll(".cart-list__body").forEach((item => {
                            const itemId = item.querySelector(".id-delete-btn").textContent;
                            if (itemId === productId) item.remove();
                        }));
                        updateCartTotal();
                    }
                }
